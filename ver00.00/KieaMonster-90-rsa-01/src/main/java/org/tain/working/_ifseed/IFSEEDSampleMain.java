...
public class IFSEEDSampleMain
{
	...
	private byte[] localASKey = new byte[16];			// 송신기관 대칭키(OTK)
	...
	public void start() throws Exception {
		...
		//byte[] b64PubKey = org.apache.commons.codec.binary.Base64.encodeBase64(bPubKey);
		//byte[] b64PriKey = org.apache.commons.codec.binary.Base64.encodeBase64(bPriKey);
		if (!Boolean.TRUE) {
			/*
			System.out.println(">>>>> Public Key(Base64) <<<<<");
			StringTools.printHex(b64PubKey);
			System.out.println(">>>>> Private Key(Base64) <<<<<");
			StringTools.printHex(b64PriKey);
			*/
		}
		...
		System.out.println("[공개키(BASE64)]");
		StringTools.printHex(b64RemotePubKey);
		System.out.println("[공개키]");
		StringTools.printHex(bRemotePubKey);
		...
		// 대칭키(Symmetric Key) = 이니셜벡터(Initial Vector) = 원타임키(One Time Key)
		new Random().nextBytes(localASKey);
		localASKey = "20210513HW202135".getBytes();  // TODO-KANG20210513
		...
		StringTools.printHex(localASKey);
		System.out.println();
		...
		System.out.println("[자료원본] " + myMsg);
		byte[] bMyMsg = myMsg.getBytes(ENCODE_STR);
		System.out.println("[자료원본(byte)]");
		StringTools.printHex(bMyMsg);

		// 자료 암호화
		// 전 참가기관은 자료를 동일한 인코딩 방식으로 송수신
		byte[] bEncMyMsg = seedEncrypt(localASKey, localIv, bMyMsg);
		System.out.println("[암호화(SEED) 자료원본(byte)]");
		StringTools.printHex(bEncMyMsg);

		// BASE64 인코딩
		//byte[] b64EncMyMsg = Base64.getEncoder().encode(bEncMyMsg);
		byte[] b64EncMyMsg = org.apache.commons.codec.binary.Base64.encodeBase64(bEncMyMsg);
		System.out.println("[BASE64 암호화(SEED) 자료원본(byte)]");
		StringTools.printHex(b64EncMyMsg);
		...
		StringTools.printHex(b64EncASKey);
		System.out.println();
		...
		System.out.println("[암호(PriKey) 대칭키]");
		StringTools.printHex(bEncRemoteASKey);
		System.out.println("[암호(SEED) 원문]");
		StringTools.printHex(bEncRemoteMsg);
		System.out.println();
		...
		System.out.println("[대칭키 = 이니셜벡터]");
		StringTools.printHex(remoteASKey);
		...
		System.out.println("[수신된 원문(byte)]");
		StringTools.printHex(bRemoteMsg);
		...
		System.out.println("[수신된 원문(String)]");
		//for (int i = 0; i < bEncRemoteMsg.length; i++) System.out.print(Integer.toHexString(0xff & bEncRemoteMsg[i])+" ");
		System.out.println("[원문] " + remoteMsg);
		System.out.println();
	 ...
	 */
	public PublicKey getPublicKey(byte[] bPubKey) throws NoSuchAlgorithmException, InvalidKeySpecException {
	 ...
	 */
	public byte[] RSAEncrypt(PublicKey pubKey, byte[] bPlainText) throws NoSuchAlgorithmException, NoSuchProviderException, NoSuchPaddingException, InvalidKeyException, IllegalBlockSizeException, BadPaddingException {
	 ...
	 * @throws BadPaddingException
	 */
	public byte[] RSADecrypt(PrivateKey priKey, byte[] bEncText) throws InvalidKeyException, NoSuchAlgorithmException, NoSuchProviderException, NoSuchPaddingException, IllegalBlockSizeException, BadPaddingException {
		Cipher cipher = Cipher.getInstance(PUB_CIPHER_ALGORITHM);
