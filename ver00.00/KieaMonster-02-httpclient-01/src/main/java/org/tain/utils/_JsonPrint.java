...
import org.tain.utils.enums._JsonPrintType;
...
@Deprecated
@Slf4j
public class _JsonPrint implements _JsonPrintImpl {

	private static _JsonPrintType oldType;
	private static _JsonPrint instance = null;
	
	public static _JsonPrint getInstance() {
		return getInstance(_JsonPrintType.NORMAL);
	}
	
	public static _JsonPrint getInstance(_JsonPrintType type) {
		if (instance == null || oldType != type) {
			instance = new _JsonPrint(type);
			oldType = type;
		}
		return instance;
	}
	
	private _JsonPrint(_JsonPrintType type) {
