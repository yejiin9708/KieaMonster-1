package org.tain.working.campaign;
...
public class CampaignWorking {
	...
	public void selectAllAndSend() throws Exception {
		...
		if (Flag.flag) {
			...
			info.put("url", "http://localhost:8080/v0.1/rest/campaign/list");
			...
			//
			if (Flag.flag) {
				ArrayNode arrCampaign = recv.getArrayNode("_response");
				System.out.println(">>>>> arrCampaign.size() = " + arrCampaign.size());
				//JsonNode board = arrCampPage.get(0);
				this.index = 0;
				arrCampaign.forEach(jsonNode -> {
	...
	private void post(JsonNode jsonNode) throws Exception {
		...
		if (Flag.flag) {
			...
			// 
			MonJsonNode info = new MonJsonNode("{}");
			info.put("url", "http://localhost:8083/v0.1/api/campaigns");
